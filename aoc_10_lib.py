...
def calc_slope(direction: Tuple[int, int], height: int) -> float:
    """"""
    if direction[0] == 0:
        slope = 2 * height * sign(direction[1])  # Greater than any possible slope
    else:
...
def create_slope_dct_quadrant(grid_dct: Dict[Tuple[int, int], int],
    ...
    for index_width, index_height in itertools.product(range(quadrant[0][0], quadrant[0][1]),
        ...
        if grid_dct[point] and station_point != point:
            direction: Tuple[int, int] = calc_direction(station_point, point)[1:]
            if direction in slope_dct_quadrant:
                slope_dct_quadrant[direction].append(point)
            else:
                slope_dct_quadrant[direction] = [point]

    slope_dct_quadrant = dict(sorted(slope_dct_quadrant.items(), key=lambda item: calc_slope(item[0], height)))
    for lst in slope_dct_quadrant.values():
        lst.sort(key=lambda point_2: (point_2[0]-station_point[0])**2 + (point_2[1]-station_point[1])**2)
...
def part_2(data: List[List[int]]) -> int:
    ...
    quadrants: List[List[Tuple[int, int]]] = [[(station_point[0], width), (0, station_point[1])],
                                              [(station_point[0], width), (station_point[1], height)],
                                              [(0, station_point[0]), (station_point[1], height)],
